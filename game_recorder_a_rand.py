...
#has no UI but does have ability to choose wuich player will be auto and which will be random
...
#decreases with each round change
rounds = 1
...
def change_value_matrix(r,c,val, b):
    ...
    if (turn_count//num_open >= total_rounds):
        ended=1
        #freeze_all()
        #print(result_arr)
        global root
    ...
    if (auto_player == curr_player):
        ...
        #time.sleep(0)
        auto_move = decide_random(curr_player)
        change_value_matrix(auto_move[0],auto_move[1], auto_move[2], button_matrix[auto_move[0]][auto_move[1]])
