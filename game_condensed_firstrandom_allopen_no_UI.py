...
def change_value_matrix(r,c,val, b):
    ...
    if (auto_player == curr_player):
        ...
        auto_move = decide_random(auto_player)
...
def fill_inner(val):
    ...
    excel_list = excel_list + get_pos_val_concat([3,4,1])
    record_scores()

    value_matrix[5][4] = 1
    excel_list.append(p2_letter)
    excel_list = excel_list + get_pos_val_concat([5,4,1])
    record_scores()

    value_matrix[4][3] = 1
    excel_list.append(p1_letter)
    excel_list = excel_list + get_pos_val_concat([4,3,1])
    record_scores()

    value_matrix[4][5] = 1
    excel_list.append(p2_letter)
    excel_list = excel_list + get_pos_val_concat([4,5,1])
    record_scores()
